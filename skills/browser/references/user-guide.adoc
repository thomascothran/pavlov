...
(e/with-chrome-headless driver
  ...
  ;; run the rest of the examples in this body
  )
----
...
To prevent that, this guide standardizes on `with-chrome-headless`:
...
TIP: In this guide, use `with-chrome-headless` for examples so cleanup always happens.

Let's say we want to create a chrome headless driver safely:
...
(e/with-chrome-headless {:args ["--window-size=1280,800"]} driver
  ;; do stuff
  )
----

The `with-chrome-headless` function handles cleanup nicely:
...
Use this pattern for browser sessions in this skill.
...
[source,clojure]
----
(e/with-chrome-headless {:load-strategy :none} driver
  (e/go driver sample-page)
...
(e/with-chrome-headless {:dev {}} driver
  ;; do some stuff
)
----
...
(e/with-chrome-headless {:dev {:perf {:level :all
                                      :network? true
                                      :page? true
                                      :interval 1000
                                      :categories [:devtools
...
// Cleanup is handled by with-chrome-headless in examples above.
...
[source,clojure]
----
(e/with-chrome-headless {:path-driver "/Users/ivan/downloads/chromedriver"} driver
  ;; do stuff
  )
----
...
;; with extra settings
(e/with-chrome-headless {:log-level :all} driver
  (e/go driver "https://clojure.org"))
----
...
[source,clojure]
----
(e/with-chrome-headless {:download-dir "target/etaoin-play/chrome-downloads"} driver
  ;; do some downloading
  )
----
...
[source,clojure]
----
(e/with-chrome-headless {:download-dir "target/etaoin-play/firefox-downloads"
...
[source,clojure]
----
(e/with-chrome-headless {:user-agent "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"}
                        driver
...
;; example
(e/with-chrome-headless {:proxy {:http "some.proxy.com:8080"
                                 :ssl "some.proxy.com:8080"}}
                        driver
  ;; do stuff
  )
...
[source,clojure]
----
(e/with-chrome-headless {:capabilities
                         {:proxy
                          {:proxyType "manual"
...
(e/with-chrome-headless {:host "127.0.0.1" :port 9515} driver
  ;; do stuff
  ) ;; for connection to driver on localhost on port 9515
...
(e/with-chrome-headless {:webdriver-url "https://chrome.browserless.io/webdriver"
...
(e/with-chrome-headless {:profile chrome-profile} chrome-driver
  ;; do stuff
  )
...
(e/with-chrome-headless {:profile ff-profile} firefox-driver
  ;; do stuff
  )
----
...
(e/with-chrome-headless {:download-dir DL-DIR} driver
  ;; Later, in tests...
...
[source,clojure]
----
(e/with-chrome-headless {:host "localhost" :port 9515 :args ["--no-sandbox"]} driver
  ;; do stuff
  )
----
...
[source,clojure]
----
(e/with-chrome-headless {:log-stdout "ffout.log" :log-stderr "fferr.log" :driver-log-level "trace"}
                        driver
  ;; inspect logs after running steps
  )
----
